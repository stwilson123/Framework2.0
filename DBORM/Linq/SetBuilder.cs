...
namespace DBORM
{
    ...
    public class SetBuilder<T> where T : IPropertyChanged,new()
	{
        ...
        public SetBuilder(IDatabase db)
		{
			__db = db;
        ...
        /// <param name="expression">值</param>
        /// <returns>更新构建器</returns>
        public SetBuilder<T> Set(string property, Expression expression)
        {
            __set.Append(property, expression);
        ...
        /// <param name="expression">值</param>
        /// <returns>更新构建器</returns>
        public SetBuilder<T> Set(string property, object value)
        {
            __set.Append(property, value);
        ...
        public SetBuilder<T> Set(Expression<Func<T, T>> SetExpression)  
        {
            MemberInitExpression expression = SetExpression.Body as MemberInitExpression;
            if (expression.Bindings != null && expression.Bindings.Count > 0)
            {
                for (int i = 0; i < expression.Bindings.Count; i++)
                {
                    ...
                    {
                        var assignMemnt = ((MemberAssignment)expression.Bindings[i]).Expression;
                        this.Set(expression.Bindings[i].Member.Name, assignMemnt);
                    }
