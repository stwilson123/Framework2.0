...
namespace DBORM
{
	...
	internal class WhereBuilder : ExpressionVisitor
	{
		...
		public WhereBuilder(IDatabase db, List<object> args = null, bool withTableName = true)
		{
		...
		protected override Expression VisitBinary(BinaryExpression expression)
		{
            ...
            List<ExpressionType> et = new List<ExpressionType>() { ExpressionType.Constant, ExpressionType.Convert };
            if (et.Contains(expression.Right.NodeType))
            {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression expression, object memberValue = default(object))
		{
			switch(expression.Method.Name)
			{
				...
				case "EndsWith":
					...
					break;
				case "NotContains":
					this.ParseNotLike(expression, "Contains");
					break;
				case "NotStartsWith":
					this.ParseNotLike(expression, "StartsWith");
					break;
				case "NotEndsWith":
					this.ParseNotLike(expression, "EndsWith");
					break;
				case "In_":
        ...
        private string GetColumnName(string columnName)
        {
            ...
            //	return "{0}.{1}".F(__db.GetTableName(expression.Member.ReflectedType), colName);
            //return colName;
            return __wtn ? __db.GetTableAndColumnName(columnName) : __db.GetColumnName(columnName);
        }
		...
		private void ParseIn(MethodCallExpression expression)
		{
			...
			if(value is IList)
			{
                ...
                if(val.Count > 0)
                {
                    ...
                    {
                        __sql.Append("(");
                        var listSplitVal = CollectionHelper.SplitCollection(val, 400);
                        foreach (var splitValTmp in listSplitVal)
                        {
                            __sql.AppendFormat("({0} IN (@{1}))" + (!listSplitVal.IsLastItem_(splitValTmp) ? " OR " : ""), this.GetColumnName(column), this.Params.Count);
                            this.Params.Add(val);
                        }
                        __sql.Append(")");
                    }
		...
		private void ParseNotLike(MethodCallExpression expression, string type)
		{
            var paramCount = expression.Arguments.Count;
            if (paramCount < 1)
                throw new InvalidOperationException("less than one argument ");

            var value = this.GetRightValue(expression.Arguments[paramCount - 1]);
		 
			__sql.AppendFormat("({0} NOT LIKE @{1})", this.GetColumnName((MemberExpression)(expression.Arguments[0])), this.Params.Count);
			switch(type)
			{
				case "Contains":
					value = "%" + value.ToString().Trim('%') + "%";
