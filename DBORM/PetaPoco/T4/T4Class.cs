...
namespace DBEntityGenerate.PetaPoco.T4
{
    class T4Class
    {
        /*
     ...
     "stackalloc", "else", "long", "static", "enum", "namespace", "string" };
        ...
        Tables LoadTables()
        {
            ...
            catch (Exception x)
            {
                ...
                sb.AppendLine(string.Format("Failed to load provider `{0}` - {1}", ProviderName, error));
                sb.AppendLine("");
                sb.AppendLine("// -----------------------------------------------------------------------------------------");
                sb.AppendLine(string.Format("// Failed to load provider `{0}` - {1}", ProviderName, error));
                sb.AppendLine("// -----------------------------------------------------------------------------------------");
            ...
            catch (Exception x)
            {
                ...
                StringBuilder sb = new StringBuilder();
                sb.AppendLine(string.Format("Failed to read database schema - {0}", error));
                sb.AppendLine("");
                sb.AppendLine("// -----------------------------------------------------------------------------------------");
                sb.AppendLine(string.Format("// Failed to read database schema - {0}", error));
                sb.AppendLine("// -----------------------------------------------------------------------------------------");
        ...
        class SqlServerSchemaReader : SchemaReader
        {
                ...
                using (var cmd = _factory.CreateCommand())
                {
                    ...
                    using (System.Data.IDataReader rdr = cmd.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            ...
                            col.PropertyType = GetPropertyType(rdr["DataType"].ToString(), (rdr["NumericScale"] == DBNull.Value ? null : rdr["NumericScale"].ToString()));
            ...
            string GetPropertyType(string sqlType, string dataScale)
            {
                ...
                if (sqlType == "numeric" && dataScale == "0")
                    return "long";
                return sysType;
            }
                ...
                using (var cmd = _factory.CreateCommand())
                {
                    ...
                    using (IDataReader rdr = cmd.ExecuteReader())
                    {
                        while (rdr.Read())
                        {
                            ...
                            //Comment by 何权洲
                            //Oracle获取自增列，需要查Sequence是否存在，暂时用简化方式处理，将IsAutoIncrement默认是false，
