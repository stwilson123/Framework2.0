    ...
    public class DefaultExtensionCompiler : IExtensionCompiler {
        ...
        public void Compile(CompileExtensionContext context) {
            ...
            try {
                var projectFileDescriptor = _projectFileParser.Parse(context.VirtualPath);
                    ...
                    foreach (var assemblyReference in projectFileDescriptor.References) {
                        ...
                        if (assembly != null) {
                            ...
                            throw new SystemCoreException(T("Assembly reference '{0}' for project '{1}' cannot be loaded", assemblyReference.FullName, context.VirtualPath));
                        }
                    }
                }
            catch (Exception ex) {
                ...
                throw new SystemCoreException(T("Error compiling module \"{0}\" from file \"{1}\":\r\n{2}", moduleName, context.VirtualPath, ex.Message), ex);
            }
        }
